...
(median ~13,400 after 90 updates).

Re-seeded 2026-07-31, when the scripted opponent was ported to the real game
robot's behaviour: the previously hunted seed stopped producing the exploit
against the corrected L3. The phenomenon itself got STURDIER rather than more
fragile. Four of the first eight seeds tried now clear the bar, three of them
by two orders of magnitude, where the old demo rested on a single hunted seed.
"""
...
def test_fresh_agent_learns_to_cheat():
    ...
    net = X.train_short_ppo(seed=2, rules="v1-freewalls", level=3,
                            updates=90, log=None)
